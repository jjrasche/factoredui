-- observe schema: pg_cron scheduled jobs
...
SELECT cron.schedule(
  ...
  '0 * * * *',
  $$
    REFRESH MATERIALIZED VIEW CONCURRENTLY observe.mv_alarm_completion;
    REFRESH MATERIALIZED VIEW CONCURRENTLY observe.mv_alarm_error_rate;
    REFRESH MATERIALIZED VIEW CONCURRENTLY observe.mv_alarm_drop_off;
    REFRESH MATERIALIZED VIEW CONCURRENTLY observe.mv_diagnostic_retries;
    REFRESH MATERIALIZED VIEW CONCURRENTLY observe.mv_diagnostic_rage_clicks;
    REFRESH MATERIALIZED VIEW CONCURRENTLY observe.mv_diagnostic_dead_clicks;
    REFRESH MATERIALIZED VIEW CONCURRENTLY observe.mv_diagnostic_scroll_reversals;
    REFRESH MATERIALIZED VIEW CONCURRENTLY observe.mv_diagnostic_hesitation;
  $$
);

-- Hourly +5min: upsert computed factors from unified view into factors table
SELECT cron.schedule(
  'observe-upsert-factors',
  '5 * * * *',
  $$
    INSERT INTO observe.factors (user_id, component_path, factor_name, factor_tier, value, computed_at)
...
SELECT cron.schedule(
  'observe-snapshot-factors',
  '0 2 * * *',
  $$
    INSERT INTO observe.factor_snapshots (user_id, component_path, factor_name, factor_tier, value, computed_at, snapshot_at)
    SELECT user_id, component_path, factor_name, factor_tier, value, computed_at, now()
    FROM observe.factors;
  $$
);
...
SELECT cron.schedule(
  'observe-compute-vectors',
  '30 2 * * *',
  $$
    INSERT INTO observe.user_factor_vectors (user_id, vector, updated_at)
    SELECT
      f.user_id,
      ARRAY[
...
SELECT cron.schedule(
  'observe-cluster-users',
  '0 3 * * *',
  $$
    SELECT net.http_post(
      url := current_setting('app.supabase_url') || '/functions/v1/observe-cluster',
