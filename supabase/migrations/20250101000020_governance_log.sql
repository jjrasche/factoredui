-- observe schema: append-only governance audit log
-- Records every automated evaluation so governors can audit decisions.

CREATE TABLE observe.governance_log (
  id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  experiment_id uuid NOT NULL REFERENCES observe.experiments(id),
...
CREATE INDEX idx_governance_log_experiment
  ON observe.governance_log (experiment_id, evaluated_at DESC);

-- Append-only: no UPDATE or DELETE via RLS
ALTER TABLE observe.governance_log ENABLE ROW LEVEL SECURITY;

-- Service role (cron) can insert
CREATE POLICY governance_log_insert_service
  ON observe.governance_log
  FOR INSERT
  TO service_role
  WITH CHECK (true);
...
GRANT SELECT ON observe.governance_log TO authenticated;
GRANT ALL ON observe.governance_log TO service_role;
GRANT SELECT ON observe.v_experiment_summary TO authenticated;
GRANT SELECT ON observe.v_experiment_summary TO service_role;
