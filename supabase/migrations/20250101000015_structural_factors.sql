-- observe schema: structural factor materialized views
...
CREATE MATERIALIZED VIEW observe.mv_structural_component_depth AS
SELECT
  e.user_id,
  e.component_path,
  'component_depth'::text AS factor_name,
  'structural'::observe.factor_tier AS factor_tier,
...
CREATE UNIQUE INDEX ON observe.mv_structural_component_depth (user_id, component_path);


-- Structural: sibling count at the component's level
CREATE MATERIALIZED VIEW observe.mv_structural_sibling_count AS
SELECT
  e.user_id,
  e.component_path,
...
CREATE UNIQUE INDEX ON observe.mv_structural_sibling_count (user_id, component_path);


-- Structural: form field count within the component
CREATE MATERIALIZED VIEW observe.mv_structural_form_field_count AS
SELECT
  e.user_id,
  e.component_path,
...
CREATE UNIQUE INDEX ON observe.mv_structural_form_field_count (user_id, component_path);
...
CREATE OR REPLACE VIEW observe.v_factors_current AS
  ...
  UNION ALL
  SELECT user_id, component_path, factor_name, factor_tier, value, computed_at
    FROM observe.mv_structural_component_depth
  UNION ALL
  SELECT user_id, component_path, factor_name, factor_tier, value, computed_at
    FROM observe.mv_structural_sibling_count
  UNION ALL
  SELECT user_id, component_path, factor_name, factor_tier, value, computed_at
    FROM observe.mv_structural_form_field_count;
...
BEGIN
  ...
  REFRESH MATERIALIZED VIEW observe.mv_structural_component_depth;
  REFRESH MATERIALIZED VIEW observe.mv_structural_sibling_count;
  REFRESH MATERIALIZED VIEW observe.mv_structural_form_field_count;
END;
$$;
