-- observe schema: governance support
...
CREATE OR REPLACE VIEW observe.v_experiment_summary AS
SELECT
...
LEFT JOIN observe.experiment_variants v
  ON v.experiment_id = e.id
LEFT JOIN observe.experiment_assignments a
  ON a.experiment_id = e.id AND a.variant_key = v.variant_key
LEFT JOIN observe.experiment_exposures x
