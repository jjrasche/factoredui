-- observe schema: raw interaction events (highest volume table)
-- Immutable append-only. SDK writes, factor engine reads.

CREATE TABLE observe.events (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id         uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  session_id      uuid NOT NULL REFERENCES observe.sessions(id) ON DELETE CASCADE,
  event_type      observe.event_type NOT NULL,
