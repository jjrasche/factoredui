-- factoredui schema: factor computation views + experiment results + governor aggregates
...
CREATE MATERIALIZED VIEW factoredui.mv_alarm_completion AS
SELECT
  e_start.user_id,
  e_start.component_path,
  'completion_rate'::text AS factor_name,
  'alarm'::factoredui.factor_tier AS factor_tier,
  ...
  now() AS computed_at
FROM factoredui.events e_start
LEFT JOIN factoredui.events e_end
...
CREATE UNIQUE INDEX ON factoredui.mv_alarm_completion (user_id, component_path);


-- Alarm: error rate (error events / total events)
CREATE MATERIALIZED VIEW factoredui.mv_alarm_error_rate AS
SELECT
  e.user_id,
  e.component_path,
  ...
  now() AS computed_at
FROM factoredui.events e
WHERE e.created_at > now() - interval '7 days'
GROUP BY e.user_id, e.component_path
WITH NO DATA;

CREATE UNIQUE INDEX ON factoredui.mv_alarm_error_rate (user_id, component_path);


-- Diagnostic: rage clicks
CREATE MATERIALIZED VIEW factoredui.mv_diagnostic_rage_clicks AS
SELECT
  e.user_id,
  e.component_path,
  'rage_click_count'::text AS factor_name,
  'diagnostic'::factoredui.factor_tier AS factor_tier,
...
CREATE UNIQUE INDEX ON factoredui.mv_diagnostic_rage_clicks (user_id, component_path);


-- Diagnostic: dead clicks
CREATE MATERIALIZED VIEW factoredui.mv_diagnostic_dead_clicks AS
SELECT
  e.user_id,
  e.component_path,
...
CREATE UNIQUE INDEX ON factoredui.mv_diagnostic_dead_clicks (user_id, component_path);


-- Diagnostic: scroll reversals
CREATE MATERIALIZED VIEW factoredui.mv_diagnostic_scroll_reversals AS
SELECT
  e.user_id,
  e.component_path,
...
CREATE UNIQUE INDEX ON factoredui.mv_diagnostic_scroll_reversals (user_id, component_path);


-- Diagnostic: hesitation (avg ms from impression to first interaction)
CREATE MATERIALIZED VIEW factoredui.mv_diagnostic_hesitation AS
SELECT
  imp.user_id,
  imp.component_path,
  ...
  now() AS computed_at
FROM factoredui.events imp
INNER JOIN LATERAL (
  SELECT e2.created_at
  FROM factoredui.events e2
...
CREATE UNIQUE INDEX ON factoredui.mv_diagnostic_hesitation (user_id, component_path);


-- Alarm: drop-off rate (sessions with impression but no completion or navigation away)
CREATE MATERIALIZED VIEW factoredui.mv_alarm_drop_off AS
SELECT
  e.user_id,
  e.component_path,
...
CREATE UNIQUE INDEX ON factoredui.mv_alarm_drop_off (user_id, component_path);


-- Diagnostic: retry count (repeated submit/click actions on same component within a session)
CREATE MATERIALIZED VIEW factoredui.mv_diagnostic_retries AS
SELECT
  e.user_id,
  e.component_path,
...
CREATE UNIQUE INDEX ON factoredui.mv_diagnostic_retries (user_id, component_path);


-- Unified view: all current factors from all MVs
CREATE OR REPLACE VIEW factoredui.v_factors_current AS
  SELECT user_id, component_path, factor_name, factor_tier, value, computed_at
    FROM factoredui.mv_alarm_completion
  UNION ALL
  SELECT user_id, component_path, factor_name, factor_tier, value, computed_at
    FROM factoredui.mv_alarm_error_rate
  UNION ALL
  SELECT user_id, component_path, factor_name, factor_tier, value, computed_at
    FROM factoredui.mv_alarm_drop_off
  UNION ALL
  SELECT user_id, component_path, factor_name, factor_tier, value, computed_at
    FROM factoredui.mv_diagnostic_retries
  UNION ALL
  SELECT user_id, component_path, factor_name, factor_tier, value, computed_at
    FROM factoredui.mv_diagnostic_rage_clicks
  UNION ALL
  SELECT user_id, component_path, factor_name, factor_tier, value, computed_at
    FROM factoredui.mv_diagnostic_dead_clicks
  UNION ALL
  SELECT user_id, component_path, factor_name, factor_tier, value, computed_at
    FROM factoredui.mv_diagnostic_scroll_reversals
  UNION ALL
  SELECT user_id, component_path, factor_name, factor_tier, value, computed_at
    FROM factoredui.mv_diagnostic_hesitation;
...
CREATE OR REPLACE VIEW factoredui.v_experiment_results AS
SELECT
  ea.experiment_id,
  ea.variant_key,
  ea.user_id,
...
FROM factoredui.experiment_assignments ea
JOIN factoredui.experiments exp ON exp.id = ea.experiment_id
CROSS JOIN LATERAL (
  SELECT DISTINCT fs.factor_name, fs.factor_tier
  FROM factoredui.factor_snapshots fs
...
CREATE OR REPLACE VIEW factoredui.v_component_factors_agg AS
SELECT
  component_path,
  factor_name,
  factor_tier,
...
FROM factoredui.v_factors_current
...
REFRESH MATERIALIZED VIEW factoredui.mv_alarm_completion;
REFRESH MATERIALIZED VIEW factoredui.mv_alarm_error_rate;
REFRESH MATERIALIZED VIEW factoredui.mv_alarm_drop_off;
REFRESH MATERIALIZED VIEW factoredui.mv_diagnostic_retries;
REFRESH MATERIALIZED VIEW factoredui.mv_diagnostic_rage_clicks;
REFRESH MATERIALIZED VIEW factoredui.mv_diagnostic_dead_clicks;
REFRESH MATERIALIZED VIEW factoredui.mv_diagnostic_scroll_reversals;
REFRESH MATERIALIZED VIEW factoredui.mv_diagnostic_hesitation;
