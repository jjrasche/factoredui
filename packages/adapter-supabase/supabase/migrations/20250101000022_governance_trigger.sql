-- factoredui schema: event-driven governance evaluation
...
CREATE OR REPLACE FUNCTION factoredui.evaluate_governance_for_experiment(
...
BEGIN
  ...
  FROM factoredui.experiments e
  WHERE e.id = p_experiment_id
    AND e.status = 'running'
    AND EXISTS (
      SELECT 1 FROM factoredui.thresholds t
  ...
  IF v_control_users IS NULL OR v_factor_names IS NULL THEN
    INSERT INTO factoredui.governance_log (experiment_id, verdict, factor_verdicts)
  ...
  LOOP
    ...
    FROM factoredui.bulk_factor_deltas(
  ...
  INSERT INTO factoredui.governance_log (experiment_id, verdict, winning_variant, factor_verdicts)
  VALUES (exp.id, v_verdict, v_winner, v_factor_verdicts);

  IF v_verdict = 'conclude' THEN
    UPDATE factoredui.experiments
...
CREATE OR REPLACE FUNCTION factoredui.on_factor_snapshot_insert()
...
BEGIN
  ...
  LOOP
    ...
    FROM factoredui.governance_log gl
    ...
    PERFORM factoredui.evaluate_governance_for_experiment(v_experiment.id);
  END LOOP;
...
CREATE TRIGGER trg_factor_snapshot_governance
  AFTER INSERT ON factoredui.factor_snapshots
  FOR EACH ROW
  EXECUTE FUNCTION factoredui.on_factor_snapshot_insert();

-- Step 4: Refactor evaluate_governance() to delegate to the per-experiment function
CREATE OR REPLACE FUNCTION factoredui.evaluate_governance()
RETURNS void LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
  ...
  LOOP
    PERFORM factoredui.evaluate_governance_for_experiment(v_experiment_id);
  END LOOP;
END;
$$;
