-- factoredui schema: append-only governance audit log
-- Records every automated evaluation so governors can audit decisions.

CREATE TABLE factoredui.governance_log (
  id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  experiment_id uuid NOT NULL REFERENCES factoredui.experiments(id),
...
CREATE INDEX idx_governance_log_experiment
  ON factoredui.governance_log (experiment_id, evaluated_at DESC);

-- Append-only: no UPDATE or DELETE via RLS
ALTER TABLE factoredui.governance_log ENABLE ROW LEVEL SECURITY;

-- Service role (cron) can insert
CREATE POLICY governance_log_insert_service
  ON factoredui.governance_log
  FOR INSERT
  TO service_role
  WITH CHECK (true);
...
GRANT SELECT ON factoredui.governance_log TO authenticated;
GRANT ALL ON factoredui.governance_log TO service_role;
GRANT SELECT ON factoredui.v_experiment_summary TO authenticated;
GRANT SELECT ON factoredui.v_experiment_summary TO service_role;
