-- factoredui schema: SDUI spec storage
...
CREATE TABLE factoredui.ui_specs (
...
CREATE INDEX idx_ui_specs_version
  ON factoredui.ui_specs (spec_version DESC);

ALTER TABLE factoredui.ui_specs ENABLE ROW LEVEL SECURITY;

COMMENT ON TABLE factoredui.ui_specs IS
  'Signed declarative UI specs. Each version is immutable once stored.';

COMMENT ON COLUMN factoredui.ui_specs.created_by IS
  'Who created this spec: governance, experiment:<id>, manual, llm';

COMMENT ON COLUMN factoredui.ui_specs.spec_hash IS
  'SHA-256 hash of component_tree JSON for integrity verification';

COMMENT ON COLUMN factoredui.ui_specs.signature IS
  'Ed25519 signature of spec_hash for authenticity verification';


CREATE TABLE factoredui.ui_active (
  platform        text PRIMARY KEY,
  spec_id         uuid NOT NULL REFERENCES factoredui.ui_specs(id),
  activated_at    timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE factoredui.ui_active ENABLE ROW LEVEL SECURITY;

COMMENT ON TABLE factoredui.ui_active IS
  'Which spec version is currently active per platform. Updated by governance.';


-- Grants
GRANT SELECT ON factoredui.ui_specs TO authenticated;
GRANT ALL ON factoredui.ui_specs TO service_role;
GRANT SELECT ON factoredui.ui_active TO authenticated;
GRANT ALL ON factoredui.ui_active TO service_role;
