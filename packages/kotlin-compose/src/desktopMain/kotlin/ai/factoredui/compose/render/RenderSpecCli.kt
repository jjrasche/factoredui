...
// args = <spec-json-path> <out-png-path> [width] [height] [density]. Exit 0 + PNG on disk, else nonzero.
fun main(args: Array<String>) {
    if (args.size < 2) {
        System.err.println("usage: render-spec-cli <spec.json> <out.png> [width=800] [height=1280] [density=2.0]")
    ...
    val png = renderSpecToPng(specFile.readText(), width = width, height = height, density = density)
    if (png.isEmpty()) {
